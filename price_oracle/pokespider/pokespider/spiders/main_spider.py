...
from ..items import PokespiderItem

from scrapy_playwright.page import PageMethod

import logging

# Headless stub (wx removed). Always selects provided set list.
class SelectionWindow:
    def __init__(self, set_names=None, grid_width=4):
        self.selection_dict = {}
        for index, set_name in enumerate(set_names or []):
            self.selection_dict[index] = [set_name, True]

    def get_selection(self):
        return [value[0] for value in self.selection_dict.values() if value[1]]
...
class MainSpider(Spider):
    ...
    def start_requests(self):
        """Kick off scraping directly for configured sets (headless, no UI)."""
        base_url = "https://www.tcgplayer.com/search/pokemon/product?productLineName=pokemon&view=grid&page=1"
        selected_sets = self.settings.getlist("DEFAULT_SET_LIST") or []
        for set_name in selected_sets:
            url_param = self.set_name_to_url_param(set_name)
            url = f"{base_url}&{url_param}"
            meta = {"card_set": set_name}
            yield self.request_search_page(url, None, meta=meta)
