    ...
    match ty {
        ...
        syn::Type::Path(path) => {
            ...
            #[cfg(procmacro2_semver_exempt)]
            {
                ...
                // Anchor IDL build runs in a host context; derive the program path from the manifest dir.
                let manifest_dir = std::env::var("CARGO_MANIFEST_DIR").unwrap_or_else(|_| ".".to_string());
                let mut source_path = std::path::PathBuf::from(&manifest_dir).join("src/lib.rs");
                if !source_path.exists() {
                    source_path = std::path::PathBuf::from(&manifest_dir).join("lib.rs");
                }
