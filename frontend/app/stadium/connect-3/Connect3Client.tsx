...
import type { Connect3Result } from "../../../stadium/games/types";

const LazyCreateGame = async (parent: HTMLElement, onRoundEnd: (r: Connect3Result) => void) => {
  const mod = await import("../../../stadium/games/connect3/createConnect3Game");
...
export default function Connect3Client() {
  const containerRef = useRef<HTMLDivElement | null>(null);
  const gameRef = useRef<Phaser.Game | null>(null);
  const [result, setResult] = useState<Connect3Result | null>(null);
    ...
    <div className="space-y-6">
      <header className="flex flex-wrap items-start justify-between gap-4 rounded-3xl border border-white/10 bg-white/5 p-5 shadow-[0_30px_90px_rgba(0,0,0,0.45)]">
        <div>
          ...
          <h1 className="text-3xl font-semibold text-white">Connect-3 Duel</h1>
          <p className="mt-2 text-sm text-white/70">Swap gems to clear lines. Bot gains score every second.</p>
        </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[2fr_1fr]">
        <div className="rounded-3xl border border-white/10 bg-black/40 p-4">
          <div className="mx-auto w-full max-w-[900px] rounded-2xl border border-white/10 bg-gradient-to-br from-white/10 via-white/5 to-transparent p-2">
          ...
          <p className="mt-3 text-center text-xs text-white/60">Swap adjacent tiles to make lines of 3 or more. Timer: 60s.</p>
        </div>
        ...
        <div className="rounded-3xl border border-white/10 bg-black/40 p-5 text-white shadow-[0_24px_70px_rgba(0,0,0,0.45)]">
          <div className="flex items-center justify-between">
            ...
            <span className="rounded-full border border-white/20 px-3 py-1 text-xs text-white/70">60s round</span>
          </div>
