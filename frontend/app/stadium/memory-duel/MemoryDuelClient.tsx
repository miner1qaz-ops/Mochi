...
import type { MemoryDuelResult } from "../../../stadium/games/types";

const LazyCreateGame = async (parent: HTMLElement, onRoundEnd: (r: MemoryDuelResult) => void) => {
  const mod = await import("../../../stadium/games/memoryDuel/createMemoryDuelGame");
...
export default function MemoryDuelClient() {
  ...
  const [result, setResult] = useState<MemoryDuelResult | null>(null);
    ...
    <div className="space-y-6">
      <header className="flex flex-wrap items-start justify-between gap-4 rounded-3xl border border-white/10 bg-white/5 p-5 shadow-[0_30px_90px_rgba(0,0,0,0.45)]">
        <div>
          ...
          <h1 className="text-3xl font-semibold text-white">Memory Duel</h1>
          <p className="mt-2 text-sm text-white/70">Flip pairs, remember positions, beat the bot’s memory.</p>
        </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[2fr_1fr]">
        <div className="rounded-3xl border border-white/10 bg-black/40 p-4">
          ...
          <p className="mt-3 text-center text-xs text-white/60">Bot remembers revealed cards; finish all pairs to win.</p>
        </div>
        ...
        <div className="rounded-3xl border border-white/10 bg-black/40 p-5 text-white shadow-[0_24px_70px_rgba(0,0,0,0.45)]">
          <div className="flex items-center justify-between">
            ...
            <span className="rounded-full border border-white/20 px-3 py-1 text-xs text-white/70">Pairs duel</span>
          </div>
          ...
          {result ? (
            <div className="mt-4 space-y-3 rounded-2xl border border-white/10 bg-white/5 p-4">
              ...
              <div className="text-sm text-white/70">
                You {result.playerPairs} — Bot {result.botPairs} of {result.totalPairs} pairs
              </div>
              <div className="text-xs text-white/60">Duration {(result.durationMs / 1000).toFixed(1)}s</div>
            </div>
          ) : (
            <div className="mt-4 rounded-2xl border border-dashed border-white/15 bg-white/5 p-4 text-sm text-white/60">
              Finish a duel to see the outcome.
            </div>
          )}
        </div>
      </div>
