...
import type { TacticsLiteResult } from "../../../stadium/games/types";

const LazyCreateGame = async (parent: HTMLElement, onRoundEnd: (r: TacticsLiteResult) => void) => {
  const mod = await import("../../../stadium/games/tacticsLite/createTacticsLiteGame");
...
export default function TacticsLiteClient() {
  const containerRef = useRef<HTMLDivElement | null>(null);
  const gameRef = useRef<Phaser.Game | null>(null);
  const [result, setResult] = useState<TacticsLiteResult | null>(null);
    ...
    <div className="space-y-6">
      <header className="flex flex-wrap items-start justify-between gap-4 rounded-3xl border border-white/10 bg-white/5 p-5 shadow-[0_30px_90px_rgba(0,0,0,0.45)]">
        <div>
          ...
          <h1 className="text-3xl font-semibold text-white">Tactics Lite</h1>
          <p className="mt-2 text-sm text-white/70">5x5 mini-scrim. Move and attack with your 3-unit squad.</p>
        </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[2fr_1fr]">
        <div className="rounded-3xl border border-white/10 bg-black/40 p-4">
          ...
          <p className="mt-3 text-center text-xs text-white/60">Select a unit, move one tile, or attack adjacent enemies. Bot moves after you.</p>
        </div>

        <div className="rounded-3xl border border-white/10 bg-black/40 p-5 text-white shadow-[0_24px_70px_rgba(0,0,0,0.45)]">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs uppercase tracking-[0.35em] text-white/50">Battle result</p>
              <h3 className="text-xl font-semibold">Latest outcome</h3>
            </div>
            <span className="rounded-full border border-white/20 px-3 py-1 text-xs text-white/70">Turn-based</span>
          </div>
          ...
          {result ? (
            <div className="mt-4 space-y-3 rounded-2xl border border-white/10 bg-white/5 p-4">
              ...
              <div className="text-sm text-white/70">
                You {result.playerUnitsRemaining} — Bot {result.botUnitsRemaining} | Turns {result.turnsTaken}
              </div>
            </div>
          ) : (
            <div className="mt-4 rounded-2xl border border-dashed border-white/15 bg-white/5 p-4 text-sm text-white/60">
              Finish a battle to see the outcome.
            </div>
          )}
        </div>
      </div>
